...
class Conv3d(Module):
    ...
    The channels are expected to be last i.e. the input shape should be ``NCDHW`` where:
        - ``N`` is the batch dimension
        - ``C`` is the number of input channels
        - ``D`` is the depth of the input volume
        - ``H`` is the input volume height
        - ``W`` is the input volume width
    ...
    def __init__(
        ...
        scale = math.sqrt(1 / (in_channels * kernel_size[0] * kernel_size[1] * kernel_size[2]))
        self.weight = mx.random.uniform(
            low=-scale,
            high=scale,
            shape=(out_channels, *kernel_size, in_channels),
